...
void bday(char *filename)
{
  ...
  printf("\t(no_bdays=%ld, no_days/yr=2^%ld,",no_bday, no_bits);
  printf(" lambda=%.2f, sample size=%ld)\n\n", lambda, no_obs);
...
#define GETDAY       ( (uni(filename) >> rt) & mask )
  ...
  for(rt=32-no_bits; rt>=0; --rt){
    ...
    printf("\t %ld to %d", 33-no_bits-rt,32-rt);
  ...
  puts("\t---------------------------------------------------------------");
  printf("\t\tp-value for KStest on those %ld p-values: %f",32-no_bits+1,pvalue);
  puts("\n"); 
