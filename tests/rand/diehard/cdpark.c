...
void park(char *filename)
{
  ...
  printf("\t\tCDPARK: result of %ld tests on file %s\n", no_obs, filename);
  printf("\t  (Of %ld tries, the average no. of successes", no_trials);
  printf(" should be \n\t   %.1f with sigma=%.1f)\n\n", mu, sigma);
...
#define CRASH   (ABS(x[k]-xtmp)<=1 && ABS(y[k]-ytmp)<=1)
  ...
  printf("\t  Square side=%ld, avg. no. parked=%.2f", side, mean);
  printf(" sample std.=%.2f\n", sqrt(var)); 
 
  pvalue=KStest(p, no_obs);
  printf("\t     p-value of the KSTEST for those %ld", no_obs);
  printf(" p-values: %f\n\n", pvalue);
